...
describe("parent push notification copy", () => {
  it("uses a neutral message for a manual total-count increase", () => {
    ...
    expect(payload.body).not.toContain("믿고 맡겨주셔서 감사드립니다");
    expect(payload.url).toBe("/p/token12345");
  });

  it("uses the payment confirmation message only when explicitly marked", () => {
    const payload = totalCountPushPayload(
      "token12345",
      "김해밀",
      108,
      120,
