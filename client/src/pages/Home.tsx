...
export default function Home() {
  ...
  const mathTestTargets = data?.mathTestTargets ?? [];
    ...
    <section className={`dashboard-workboard ${showAttendancePendingList || showJournalPendingList ? "has-pending" : ""}`} aria-label="오늘의 운영 업무판">
      <div className="dashboard-workboard-main">
        ...
        <section className="dashboard-summary-grid" aria-label="오늘의 운영 요약">
          ...
          <StatCard compact icon={BookOpenCheck} label="수학 시험 대상" value={mathTestTargets.length} suffix="명" loading={isLoading} tone="ink"><div className="mt-3 flex flex-wrap gap-1.5" aria-label="수학 시험 대상 학생 명단">{mathTestTargets.length ? mathTestTargets.map(student => <span key={student.id} className="rounded-full bg-[#F2EEE3] px-2.5 py-1 text-xs font-semibold text-[#53635F]" title={student.assessment}>{student.name}</span>) : <span className="text-xs font-medium text-[#71817D]">시험 대상 없음</span>}</div></StatCard>
        </section>
      </div>
