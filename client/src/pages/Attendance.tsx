...
import { AlertCircle, CalendarDays, CalendarPlus, Check, CheckCheck, ChevronLeft, ChevronRight, Clock3, Save } from "lucide-react";
...
type Attendance = { status: AttendanceStatus; arrivalTime: string | null; calendarEvent: CalendarEvent | null } | null;
...
export default function Attendance() {
  ...
  return <div className="journal-page-shell"><section className="journal-page-heading"><div><p className="eyebrow">WEEKLY ATTENDANCE</p><h1>출석 관리</h1><p>한 주의 출석을 한 화면에서 입력합니다. 필요한 경우 주말 수업도 함께 기록할 수 있습니다.</p></div><div className="flex flex-wrap items-center justify-end gap-2"><Button variant="outline" className="bg-white" onClick={() => setLocation("/closures")}><CalendarPlus className="mr-1.5 h-4 w-4" />휴강 관리</Button><div className="journal-date-nav"><Button variant="outline" size="icon" onClick={() => setWeekAnchor(shiftDate(weekStart, -7))} aria-label="이전 주"><ChevronLeft className="h-4 w-4" /></Button><Input type="date" value={weekStart} onChange={event => setWeekAnchor(event.target.value)} /><Button variant="outline" size="icon" onClick={() => setWeekAnchor(shiftDate(weekStart, 7))} aria-label="다음 주"><ChevronRight className="h-4 w-4" /></Button></div></div></section>
    ...
    <section className="mt-6 space-y-3">{isLoading ? Array.from({ length: 5 }).map((_, index) => <Skeleton className="h-44 w-full" key={index} />) : visibleStudents.length ? visibleStudents.map(student => <AttendanceWeekCard key={student.id} student={student} dates={visibleDates} pending={save.isPending || fillWeekdays.isPending} onSave={(journalDate, status, arrivalTime) => save.mutate({ studentId: student.id, journalDate, status, arrivalTime })} onFillWeekdays={journalDates => fillWeekdays.mutate({ studentId: student.id, journalDates })} />) : <NoStudentMessage grade={selectedGrade} attentionOnly={attentionOnly} />}</section>
  </div>;
}

function AttendanceWeekCard({ student, dates, pending, onSave, onFillWeekdays }: { student: StudentWeek; dates: string[]; pending: boolean; onSave: (date: string, status: AttendanceStatus, arrivalTime: string) => void; onFillWeekdays: (journalDates: string[]) => void }) {
...
function AttendanceDayCell({ journalDate, attendance, pending, onSave }: { journalDate: string; attendance: Attendance; pending: boolean; onSave: (status: AttendanceStatus, arrivalTime: string) => void }) {
  ...
  useEffect(() => { setStatus(attendance?.status ?? "not_entered"); setArrivalTime(attendance?.arrivalTime ? formatArrivalTimeForDisplay(attendance.arrivalTime) : ""); }, [attendance?.arrivalTime, attendance?.status, journalDate]);
  ...
  const saveCurrentStatus = () => {
    ...
    onSave(status, arrivalTime);
  };
  return <div className={`min-w-0 p-3 ${status === "not_entered" ? "bg-[#FFFDF3]" : ""}`}><div className="mb-2 flex items-center justify-between"><b className="text-xs text-[#48615C]">{dayLabel(journalDate)}</b>{status === "not_entered" && <span className="h-2 w-2 rounded-full bg-[#E1B937]" />}</div>{automaticEvent && calendarEvent && <div className="mb-2 rounded-lg border border-[#E6D9BD] bg-[#FFF9EC] px-2.5 py-2"><p className="text-xs font-semibold text-[#775A1F]">{calendarEvent.name}</p><small className="mt-0.5 block text-[10px] text-[#957842]">{calendarEvent.type === "official_holiday" ? "법정공휴일 자동 적용" : "등록한 휴강 기간 자동 적용"}</small></div>}<select value={selectValue} onChange={event => { const rawValue = event.target.value; if (rawValue === "__automatic_calendar_event__") return; const next = rawValue as AttendanceStatus; setStatus(next); if (next === "absent" || next === "not_registered" || next === "closed") setArrivalTime(""); }} className="journal-select h-9 w-full text-xs">{automaticEvent && calendarEvent && <option value="__automatic_calendar_event__" disabled>{calendarEvent.type === "official_holiday" ? "법정공휴일 (자동)" : "휴강 (자동)"}</option>}{selectableAttendanceStatusValues.map(value => <option value={value} key={value}>{attendanceStatusLabels[value]}</option>)}</select><Input disabled={disabledTime} value={arrivalTime} placeholder="오후 등원 시간 (예: 3:15)" className="mt-2 h-9 text-xs" onChange={event => { setArrivalTime(event.target.value); if (event.target.value.trim() && status === "not_entered") setStatus("present"); }} onKeyDown={event => { if (event.key === "Enter") saveCurrentStatus(); }} /><Button size="sm" disabled={pending} onClick={saveCurrentStatus} className="mt-2 h-8 w-full journal-primary-button text-xs"><Save className="mr-1 h-3.5 w-3.5" />저장</Button></div>;
}
