...
export default function Attendance() {
  ...
  return <div className="journal-page-shell"><section className="journal-page-heading"><div><p className="eyebrow">WEEKLY ATTENDANCE</p><h1>출석 관리</h1><p>한 주의 출석을 한 화면에서 입력합니다. 필요한 경우 주말 수업도 함께 기록할 수 있습니다.</p></div><div className="flex flex-wrap items-center justify-end gap-2"><Button variant="outline" className="bg-white" onClick={() => setLocation("/closures")}><CalendarPlus className="mr-1.5 h-4 w-4" />휴강 관리</Button><div className="journal-date-nav"><Button variant="outline" size="icon" onClick={() => setWeekAnchor(shiftDate(weekStart, -7))} aria-label="이전 주"><ChevronLeft className="h-4 w-4" /></Button><Input type="date" value={weekStart} onChange={event => setWeekAnchor(event.target.value)} /><Button variant="outline" size="icon" onClick={() => setWeekAnchor(shiftDate(weekStart, 7))} aria-label="다음 주"><ChevronRight className="h-4 w-4" /></Button></div></div></section>
    ...
    <section className="mt-6 space-y-3">{isLoading ? Array.from({ length: 5 }).map((_, index) => <Skeleton className="h-44 w-full" key={index} />) : visibleStudents.length ? visibleStudents.map(student => <AttendanceWeekCard key={student.id} student={student} dates={visibleDates} pending={save.isPending || fillWeekdays.isPending} onSave={(journalDate, status, arrivalTime, departureTime) => save.mutate({ studentId: student.id, journalDate, status, arrivalTime, departureTime })} onFillWeekdays={journalDates => fillWeekdays.mutate({ studentId: student.id, journalDates })} />) : <NoStudentMessage grade={selectedGrade} attentionOnly={attentionOnly} />}</section>
  </div>;
}

function AttendanceWeekCard({ student, dates, pending, onSave, onFillWeekdays }: { student: StudentWeek; dates: string[]; pending: boolean; onSave: (date: string, status: AttendanceStatus, arrivalTime: string, departureTime: string) => void; onFillWeekdays: (journalDates: string[]) => void }) {
