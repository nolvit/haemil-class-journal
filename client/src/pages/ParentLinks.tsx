...
type Student = { id: number; name: string; grade: string; familyKey: string | null; publicToken: string; portalEnabled: boolean; monthlyViewCount: number; classGroups: Array<{ id: number; name: string }> };
...
function LinkCard({ student, onCopy, onRotate, rotating }: { student: Student; onCopy: () => void; onRotate: () => void; rotating: boolean }) {
  ...
  return <Card className={`journal-surface ${student.portalEnabled ? "" : "opacity-75"}`}><CardContent className="grid gap-3 p-4 lg:grid-cols-[minmax(190px,0.65fr)_minmax(0,1fr)_auto]"><div><div className="flex items-center justify-between gap-3"><h2 className="font-serif text-xl text-[#193D3C]">{student.name}</h2><Badge className="shrink-0 bg-[#F5ECD0] text-[#765E10] hover:bg-[#F5ECD0]">이번 달 {student.monthlyViewCount ?? 0}회</Badge></div><p className="mt-1 text-xs text-[#71817D]">{student.grade} · {student.classGroups.map(group => group.name).join(" · ") || "수강 반 미지정"}</p><div className="mt-2 flex flex-wrap gap-1.5"><Badge className={`${student.portalEnabled ? "bg-[#E5F0E9] text-[#2F7154] hover:bg-[#E5F0E9]" : "bg-[#F1EEE7] text-[#796554] hover:bg-[#F1EEE7]"}`}>{student.portalEnabled ? "공개 중" : "비공개"}</Badge>{student.familyKey && <Badge className="bg-[#E7EDF7] text-[#45627A] hover:bg-[#E7EDF7]">형제·자매 공동 PWA</Badge>}</div></div><div className="flex min-w-0 items-center gap-2"><Input readOnly value={link} className="h-10 bg-[#FCFBF7] text-xs" /><Button variant="outline" size="icon" onClick={onCopy} aria-label="링크 복사"><Copy className="h-4 w-4" /></Button><a href={link} target="_blank" rel="noreferrer"><Button variant="outline" size="icon" aria-label="보호자 화면 열기"><ExternalLink className="h-4 w-4" /></Button></a></div><Button variant="outline" disabled={rotating} onClick={onRotate} className="h-10 border-[#D9C792] text-[#765E10] hover:bg-[#FFF7D9]"><RefreshCw className="mr-1.5 h-4 w-4" />링크 재발급</Button></CardContent></Card>;
}
