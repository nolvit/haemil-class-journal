...
import { attendanceStatusBadgeClass, attendanceStatusLabels, chooseJournalClassId, formatAttendanceProgressLabel, getAdjacentJournalDate, getJournalCompleteness, getMonday, initialJournalHomework, isJournalAttentionDue, isJournalHomeworkVisible, HOMEWORK_STATUS_OPTIONS, homeworkStatusDescriptions, type AttendanceStatus, type HomeworkStatusOption } from "@shared/journalRules";
...
import { carryForwardMathJournalEntries, nextMathJournalEntryForEmptyJournal, suggestMathJournalCopy } from "@shared/mathJournalCopy";
...
export default function Journal() {
  ...
  return <div className="journal-page-shell"><section className="journal-page-heading"><div><p className="eyebrow">WEEKLY LESSON JOURNAL</p><h1>수업 일지</h1><p>한 주의 수업 내용, 과제, 특이사항을 날짜별로 기록합니다. 노란색 항목은 필수 기록을 확인해야 합니다.</p></div><div className="journal-date-nav"><Button variant="outline" size="icon" onClick={() => setWeekAnchor(shiftDate(weekStart, -7))} aria-label="이전 주"><ChevronLeft className="h-4 w-4" /></Button><Input type="date" value={weekStart} onChange={event => setWeekAnchor(event.target.value)} /><Button variant="outline" size="icon" onClick={() => setWeekAnchor(shiftDate(weekStart, 7))} aria-label="다음 주"><ChevronRight className="h-4 w-4" /></Button><Button variant="ghost" size="sm" className="dashboard-today-button" onClick={() => setWeekAnchor(todayInKorea())}>오늘</Button></div></section>
    ...
    <div className="journal-guidance mt-4"><CalendarDays className="h-4 w-4" /><span><b>{weekStart} 주간 입력</b> · {attentionOnly ? `${today}까지 실제 입력이 필요한 입력 전 항목만 표시합니다.` : "이름을 누르면 4주 달력이 새 창에서 열립니다. 날짜별 셀에서는 수업일지를 작성할 수 있습니다."}</span><Badge className="ml-auto bg-[#FFF1B7] text-[#765E10] hover:bg-[#FFF1B7]">총 {visibleRecords}명</Badge></div>
...
function JournalEditor({ editing, includeWeekend, onClose }: { editing: { row: EditorRow; journalDate: string } | null; includeWeekend: boolean; onClose: () => void }) {
  ...
  const autoAdvanceAttemptRef = useRef<string | null>(null);
  ...
  const focusedLearning = trpc.academy.mathProgress.focused.useQuery(
    { studentId: row?.student.id ?? 1, throughDate: journalDate },
    { enabled: row?.classGroup.subject === "수학", staleTime: 30_000 }
  );
  ...
  useLayoutEffect(() => {
    ...
    setAutoStartedKey(null);
  }, [editing, fetchedRow, loadError, editorIdentity, content, homework, notes, mathEntries, mathSessionKind]);
  useEffect(() => {
    if (!editing) { autoAdvanceAttemptRef.current = null; return; }
    const initial = initializedEditorRef.current;
    if (!editorIdentity || !activeRow || !recentLesson.isSuccess || recentLesson.isFetching || initial?.identity !== editorIdentity ||
        autoAdvanceAttemptRef.current === editorIdentity ||
        !editorValuesEqual({ content, homework, notes, mathEntries, mathSessionKind }, initial.values)) return;
    autoAdvanceAttemptRef.current = editorIdentity;
    if (activeRow.classGroup.subject !== "수학" || !canWriteLesson || activeRow.journal ||
        !recentLesson.data || recentLesson.data.journalDate >= journalDate) return;
    const next = nextMathJournalEntryForEmptyJournal(recentLesson.data.mathProgress, journalDate);
    if (!next) return;
    setMathEntries([next]);
    setAutoStartedKey(next.key);
  }, [editing, editorIdentity, activeRow, recentLesson.isSuccess, recentLesson.isFetching, recentLesson.data, canWriteLesson, content, homework, notes, mathEntries, mathSessionKind, journalDate]);
  ...
  const save = trpc.academy.journals.save.useMutation({ onSuccess: (result, values) => { initializedEditorRef.current = null; patchJournalCaches({ ...values, content: result.content, mathProgress: result.mathProgress }); void utils.academy.weeklyWorkspace.invalidate(); void utils.academy.mathProgress.invalidate(); void utils.academy.journals.recent.invalidate(); if (!closeAfterSaveRef.current && result.mathProgress && !result.content) { setContent(result.mathProgress.freeText); setMathEntries(result.mathProgress.entries); setHomework(values.homework); } toast.success(values.isDraft ? "임시 저장했습니다. 최종 저장 전까지 입력 전으로 표시됩니다." : "수업일지를 저장했습니다."); if (closeAfterSaveRef.current) onClose(); }, onError: error => toast.error(error.message) });
  const insert = trpc.academy.journals.insert.useMutation({ onSuccess: result => { void utils.academy.weeklyWorkspace.invalidate(); void utils.academy.mathProgress.invalidate(); void utils.academy.journals.recent.invalidate(); void utils.academy.workspace.invalidate(); void utils.academy.dashboard.invalidate(); setContent(""); setHomework(initialJournalHomework(activeRow?.classGroup.subject ?? "", null, activeRow?.attendance?.status)); setNotes(""); setMathEntries([]); setMathSessionKind("math"); toast.success(result.movedCount ? `새 수업일지 자리를 추가했습니다. 저장된 일지 ${result.movedCount}건을 다음 날짜로 이동했습니다.` : "현재 날짜에 새 수업일지 입력 자리를 준비했습니다."); }, onError: error => toast.error(error.message) });
  const deleteAndPull = trpc.academy.journals.deleteAndPull.useMutation({ onSuccess: result => { void utils.academy.weeklyWorkspace.invalidate(); void utils.academy.mathProgress.invalidate(); void utils.academy.journals.recent.invalidate(); void utils.academy.workspace.invalidate(); void utils.academy.dashboard.invalidate(); setContent(""); setHomework(""); setNotes(""); setMathEntries([]); setMathSessionKind("math"); toast.success(result.movedCount ? `현재 일지를 삭제하고 미래 수업일지 ${result.movedCount}건을 앞당겼습니다.` : "현재 일지를 삭제했습니다."); onClose(); }, onError: error => toast.error(error.message) });
  ...
  const onlyAutoPreparedNext = !activeRow?.journal && autoStartedKey && mathEntries.length === 1 &&
    mathEntries[0]?.key === autoStartedKey && mathEntries[0].state === "active" &&
    content === originalContent && homework === originalHomework && notes === (activeRow?.journal?.notes ?? "") && mathSessionKind === "math";
  const hasUnsavedChanges = !onlyAutoPreparedNext && (content !== originalContent || homework !== originalHomework || notes !== (activeRow?.journal?.notes ?? "") ||
  ...
  const copyReference = () => {
    ...
    if (activeRow.classGroup.subject === "수학" && !content.trim() && autoStartedKey &&
        mathEntries.length === 1 && mathEntries[0]?.key === autoStartedKey && mathEntries[0].state === "active" &&
        reference.mathProgress?.sessionKind === "math") {
      setContent(reference.mathProgress.freeText);
      toast.success(`${reference.journalDate} 수업 설명을 가져왔습니다. 자동 준비된 다음 과정은 유지합니다.`);
      return;
    }
