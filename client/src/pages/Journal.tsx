...
function JournalStudentIdentity({ student, today, now }: { student: WeekStudent; today: string; now: Date }) {
  ...
  const progress = todayRow
    ...
    : "—";
  return <div className="px-4 py-4"><p className="font-serif text-lg text-[#193D3C]">{student.student.name}</p><p className="mt-0.5 text-xs text-[#71817D]">{student.student.grade}</p>{progress !== "—" && <p className={`mt-1 text-[11px] font-semibold ${progress === "결석" || progress === "미등록" ? "text-[#A16A3B]" : "text-[#52706A]"}`}>{progress}</p>}</div>;
}
