...
function JournalStudentIdentity({ student, classGroup, today, now }: { student: WeekStudent; classGroup: EditorRow["classGroup"]; today: string; now: Date }) {
  ...
  const progress = todayRow
    ...
    : "—";
  return <div className="px-4 py-4"><JournalHistoryButton student={student.student} classGroup={classGroup} /><p className="mt-0.5 text-xs text-[#71817D]">{student.student.grade}</p>{progress !== "—" && <p className={`mt-1 text-[11px] font-semibold ${progress === "결석" || progress === "미등록" ? "text-[#A16A3B]" : "text-[#52706A]"}`}>{progress}</p>}</div>;
}
