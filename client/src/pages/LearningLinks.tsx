...
import { BarChart3, BookMarked, Calculator, ExternalLink, Mic2, Save, ShieldAlert } from "lucide-react";
import { useEffect, useState } from "react";
import { toast } from "sonner";

type StudentLink = { id: number; name: string; grade: string; vocabularyResultUrl: string | null; englishSpeakingUrl: string | null; mathUnitEvaluationUrl: string | null; mathEvaluationSummaryUrl: string | null };

export default function LearningLinks() {
  const { user } = useAuth();
  const students = trpc.academy.students.list.useQuery({ active: true });
  if (user?.role !== "admin") return <AccessDenied />;
  return <div className="journal-page-shell"><section className="journal-page-heading"><div><p className="eyebrow">STUDENT LEARNING LINKS</p><h1>학습 링크</h1><p>학생별 학습 결과 링크를 붙여넣고 Enter로 저장합니다.</p></div></section><section className="mt-6 grid gap-3 md:grid-cols-2 xl:grid-cols-3">{students.isLoading ? Array.from({ length: 6 }).map((_, index) => <Card className="h-52 animate-pulse bg-[#FCFBF7]" key={index} />) : (students.data as StudentLink[] | undefined)?.map(student => <StudentLinkCard key={student.id} student={student} />)}</section></div>;
}
...
function StudentLinkCard({ student }: { student: StudentLink }) {
  ...
  const [mathEvaluationSummaryUrl, setMathEvaluationSummaryUrl] = useState(student.mathEvaluationSummaryUrl ?? "");
  useEffect(() => { setVocabularyResultUrl(student.vocabularyResultUrl ?? ""); setEnglishSpeakingUrl(student.englishSpeakingUrl ?? ""); setMathUnitEvaluationUrl(student.mathUnitEvaluationUrl ?? ""); setMathEvaluationSummaryUrl(student.mathEvaluationSummaryUrl ?? ""); }, [student]);
  ...
  const submit = () => save.mutate({ id: student.id, vocabularyResultUrl: vocabularyResultUrl.trim(), englishSpeakingUrl: englishSpeakingUrl.trim(), mathUnitEvaluationUrl: mathUnitEvaluationUrl.trim(), mathEvaluationSummaryUrl: mathEvaluationSummaryUrl.trim() });
  const onEnter = (event: React.KeyboardEvent<HTMLInputElement>) => { if (event.key === "Enter") { event.preventDefault(); submit(); } };
  const savedLinkCount = countSavedLearningLinks({ vocabularyResultUrl, englishSpeakingUrl, mathUnitEvaluationUrl, mathEvaluationSummaryUrl });
  return <Card className="journal-surface"><CardContent className="p-5"><div className="flex items-start justify-between gap-3"><div><p className="eyebrow">{student.grade}</p><h2 className="mt-1 font-serif text-2xl text-[#193D3C]">{student.name}</h2></div><Badge className={savedLinkCount ? "bg-[#E5F0E9] text-[#2F7154] hover:bg-[#E5F0E9]" : "bg-[#F5F1E8] text-[#776A5A] hover:bg-[#F5F1E8]"}>{savedLinkCount}개 저장</Badge></div><div className="mt-5 grid gap-3"><LinkField icon={<BookMarked className="h-4 w-4" />} label="단어 암기 결과" value={vocabularyResultUrl} onChange={setVocabularyResultUrl} onEnter={onEnter} /><LinkField icon={<Mic2 className="h-4 w-4" />} label="영어 말하기" value={englishSpeakingUrl} onChange={setEnglishSpeakingUrl} onEnter={onEnter} /><LinkField icon={<Calculator className="h-4 w-4" />} label="수학(세부)" value={mathUnitEvaluationUrl} onChange={setMathUnitEvaluationUrl} onEnter={onEnter} /><LinkField icon={<BarChart3 className="h-4 w-4" />} label="수학(요약)" value={mathEvaluationSummaryUrl} onChange={setMathEvaluationSummaryUrl} onEnter={onEnter} /></div><div className="mt-4 flex items-center justify-between"><p className="text-[11px] text-[#71817D]">붙여넣고 Enter를 누르면 저장됩니다.</p><Button variant="ghost" size="sm" onClick={submit} disabled={save.isPending}><Save className="mr-1 h-3.5 w-3.5" />저장</Button></div></CardContent></Card>;
}
